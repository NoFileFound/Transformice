    ...
    public Varargs invoke(Varargs args) {
        if (this.room.luaDebugLib != null && !this.room.luaDebugLib.checkTestCode()) {
            if (args.isnil(1)) {
                ...
                this.room.sendAll(new C_AddPhysicObject(id, bodyDef.get("dynamic").toboolean(), bodyDef.get("type").toint(), xPosition, yPosition, bodyDef.get("width").toint(), bodyDef.get("height").toint(), bodyDef.get("foreground").toboolean(), (int)(bodyDef.get("friction").tofloat() * 100), (int)(bodyDef.get("restitution").tofloat() * 100), bodyDef.get("angle").toint(), bodyDef.get("color").toint(), bodyDef.get("miceCollision").isnil() || bodyDef.get("miceCollision").toboolean(), bodyDef.get("groundCollision").isnil() || bodyDef.get("groundCollision").toboolean(), bodyDef.get("fixedRotation").toboolean(), bodyDef.get("mass").toint(), (int)(bodyDef.get("linearDamping").tofloat() * 100), (int)(bodyDef.get("angularDamping").tofloat() * 100), false, "", bodyDef.get("contactListener").isnil() || bodyDef.get("contactListener").toboolean()));
            }
        }
        return NIL;
    }
